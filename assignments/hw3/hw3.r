...
qplot(wr1500m$year, wr1500m$times_sec) + geom_point(col="red")
...
# Your qplot command:
qplot(wr1500m$new_year, wr1500m$times_sec) + geom_point()
...
# Your ggplot command:plot(wr1500m$new_year, wr1500m$times_sec,  type = "s",
p<- qplot(wr1500m$new_year, wr1500m$times_sec,
     xlim = c(min(wr1500m$new_year, na.rm=T), 2015)) + geom_point()
p+ geom_segment(aes(x=1998, xend= 2015),y=min(wr1500m$times_sec), yend=min(wr1500m$times_sec), col="red")
...
# Your ggplot command
p<- qplot(wr1500m$new_year, wr1500m$times_sec,
      xlim = c(min(wr1500m$new_year, na.rm=T), 2020)) + geom_point() + geom_vline(x=wr_1944, col="green")+ geom_vline(x=1998, col="green") +
  annotate("text", wr_1944, wr1500m$times_sec[wr1500m$new_year == wr_1944], label = wr1500m$athlete[wr1500m$new_year == wr_1944], col="blue") +
  annotate("text", wr_1998, wr1500m$times_sec[wr1500m$new_year == wr_1998], label = wr1500m$athlete[wr1500m$new_year == wr_1998], col="blue")
p+ geom_segment(aes(x=1998, xend= 2015),y=min(wr1500m$times_sec), yend=min(wr1500m$times_sec), col="red")
...
p<- qplot(wr1500m$new_year, wr1500m$times_sec,
      xlim = c(min(wr1500m$new_year, na.rm=T), 2020)) + geom_point()+ 
  labs(title  = "World Records in Men's 1500 meter") + xlab("Year") + ylab("Record Times (sec)") + 
  geom_vline(x=wr_1944, col="green")+ geom_vline(x=1998, col="green") + 
  annotate("text", wr_1944, wr1500m$times_sec[wr1500m$new_year == wr_1944], label = wr1500m$athlete[wr1500m$new_year == wr_1944], col="blue") +
  annotate("text", wr_1998, wr1500m$times_sec[wr1500m$new_year == wr_1998], label = wr1500m$athlete[wr1500m$new_year == wr_1998], col="blue")
p+geom_segment(aes(x=1998, xend= 2015),y=min(wr1500m$times_sec), yend=min(wr1500m$times_sec), col="red")
################################
...
names(SO2012Ctry)
qplot(SO2012Ctry$pop,SO2012Ctry$GDP) + geom_point() 
...
symbols(log(SO2012Ctry$pop), log(SO2012Ctry$GDP_per_person), circle=sqrt(SO2012Ctry$Total)/36, inches= FALSE)
# Your ggplot command
qplot(SO2012Ctry$pop,SO2012Ctry$GDP_per_person, size=sqrt(SO2012Ctry$Total)/36, log="xy") + geom_point()
...
qplot(log(SO2012Ctry$pop),log(SO2012Ctry$GDP_per_person), size=sqrt(SO2012Ctry$Total)/36) + geom_point() +
 labs(title="Population vs GDP per person", x="log of population", y= "log of GDP per person") + 
annotate("text", x=log(SO2012Ctry$pop)[top5], y=log(SO2012Ctry$GDP_per_person)[top5], label = SO2012Ctry$ISO[top5],
col = "red")
...
p <- ggplot()
p=p+geom_polygon(data=world,aes(x=long,y=lat,group=group),fill="grey40")
p
...
world=map_data("world")
p <- ggplot()
p=p+geom_polygon(data=world,aes(x=long,y=lat,group=group),fill="grey40", col="grey90")
p=p+geom_point(data=wonMedal, aes(longitude, latitude), size= sqrt(wonMedal$Total)/2, col="yellow")
p
## Not needed
...
p<- ggplot(data= athletes, aes(x=Sport, fill=Sex),)+ theme(axis.text.x=element_text(angle=90,hjust=1))
...
d <- function(x){
  a=sum(x>1989)-sum(x>1990)
  return(a)}
n1989.rain<- sapply(day,d)
n1989.rain
