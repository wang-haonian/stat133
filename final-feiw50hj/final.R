...
name <- "wang-haonian"
github.name <- "wang-haonian"
email.address <- "whn@berkeley.edu"
...
set.seed(42)
#z <- <your code here>
z<- sample(c(T,F),5000,replace= T, prob=c(0.3, 0.7))
...
set.seed(344)
#m <- <your code here>
m<- matrix(rnorm(100^2,8),nrow=10,byrow=F)
...
#cw2 <- <your code here>
cw2<- ChickWeight[ChickWeight$Time==0 & ChickWeight$weighed>41]
...
w4<-ChickWeight[sample(1:dim(ChickWeight),200,replace=F),]
...
t1<- with(infants, 
          table(smoke,number), useNA="ifany")
# [1 pt] 
...
fit.mtcars<- lm(mtcars$qsec~mtcars$hp)
abline(fit.mtcars)
# [3 pt]
...
plot(WorldBank$life.expectancy~WorldBank$fertility.rate, col=WorldBank$region, pch=20, cex=0.4,
...
prop.rain<- sapply(rain, function(x) length(x>0)/length(x))
...
PermDiff <- function(cases, controls, k=5000){
  Perm.Diff<-c()
  for (i in 1:k)
  total.sapmle<-c(cases,controls)
 cases.new.idc<- sample(1:(length(cases)+length(controls)),k, replace=F)
 cases.new<-total.sample(cases.new.idc)
 controls.new<- total.sample(-cases.new.idc)
 PermDiff[i]<-mean(cases.new)-mean(controls.new)
 return(PerDiff)
}
