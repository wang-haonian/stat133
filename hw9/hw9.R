...
optionlist <- list(title="mtcars", vAxis="{title: 'mpg'}", hAxis="{title:'disp'}")
bub <- gvisBubbleChart(data=mtcars, idvar="model", xvar="disp", yvar="mpg", 
    colorvar="gear", sizevar="hp", options=optionlist)
...
WorldDat <- data.frame(WorldBank$country, WorldBank$year, WorldBank$fertility.rate,
          WorldBank$life.expectancy, WorldBank$population, WorldBank$region)
...
Motion <- gvisMotionChart(data=WorldDat, idvar="WorldBank.country", 
        xvar="WorldBank.fertility.rate",
        yvar="WorldBank.life.expectancy", timevar="WorldBank.year",
        colorvar="WorldBank.region", sizevar="WorldBank.population")
